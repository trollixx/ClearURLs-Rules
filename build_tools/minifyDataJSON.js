...
const fileContent = readFileSync(inFileLocation).toString();
...
function build() {
    ...
    for (const provider in data.providers) {
        ...
        const self = minifiedData.providers[provider];
        ...
        if (
            data.providers[provider].methods &&
            data.providers[provider].methods.length !== 0
        ) {
            self.methods = data.providers[provider].methods;
        }
    }

    writeFile(outFileLocation, JSON.stringify(minifiedData), (err) => {
        if (err) {
